...
def main():
    ...
    for token, freq in tokens_c.most_common():
        terms_arr.append([token, freq, articles, math.log(float(articles) / freq)])
    terms_df = pd.DataFrame(terms_arr, columns=["token", "frequency", "total", "idf"])
    terms_df.to_csv("{}_{}".format(args.output, "terms.csv"), index=False)
    if args.stem:
        for s, freq in stems_c.most_common():
            stems_arr.append([s, freq, articles, math.log(articles / (1.0 + freq)), token_to_stem_mapping[s].most_common(1)[0][0]])
        stem_df = pd.DataFrame(stems_arr, columns=["stem", "frequency", "total", "idf", "most_freq_term"])
        stem_df.to_csv("{}_{}".format(args.output, "stems.csv"), index=False)
    return 0
